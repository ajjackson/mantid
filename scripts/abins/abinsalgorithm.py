...
class AbinsAlgorithm:
    ...
    @staticmethod
    def get_atom_selection(*, atoms_data: abins.AtomsData, selection: list) -> Tuple[List[int], List[str]]:
    ...
    def _atom_number_s(self, *, atom_number, s_data, s_atom_data, atoms_data):
        ...
        rows = filter(lambda row: row.atom_index == atom_number - 1, rows)
        # If data comes from cache, we may have rows for more quantum orders than requested: filter these out
        rows = filter(lambda row: (row.quantum_order is None) or (row.quantum_order <= self._max_event_order), rows)

        output_atom_label = f"{ATOM_PREFIX}_{atom_number}"
        s_atom_data.fill(0.0)

        for i, row in enumerate(sorted(rows, key=attrgetter("quantum_order"))):
            s_atom_data[i] = row.data

        z_number = Atom(row.atom_symbol).z_number
